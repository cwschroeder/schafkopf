...
import { WissenQuizQuestion as WissenQuizQuestionType } from '@/lib/wissen/types';
import { hapticTap, hapticSuccess, hapticError } from '@/lib/haptics';

interface WissenQuizQuestionProps {
  question: WissenQuizQuestionType;
  onAnswer: (correct: boolean) => void;
  questionNumber: number;
  totalQuestions: number;
}

export default function WissenQuizQuestion({
  question,
  onAnswer,
  questionNumber,
  totalQuestions,
}: WissenQuizQuestionProps) {
  ...
  const handleAnswer = (answer: string) => {
    ...
    // Kurze Verzoegerung vor dem Weiter
    setTimeout(() => {
      onAnswer(isCorrect);
    }, 2000);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Match Pairs (vereinfacht als Zuordnung) */}
      {question.type === 'match-pairs' && question.pairs && (
        <div className="space-y-2">
          <p className="text-amber-100/70 text-sm mb-3">
            Waehle die richtige Zuordnung:
          </p>
          {question.pairs.map((pair, index) => {
            const answerKey = `${pair.left}:${pair.right}`;
            return (
              <button
                key={index}
                onClick={() => handleAnswer(answerKey)}
                disabled={showResult}
                className={`w-full text-left p-3 rounded-lg border-2 transition-all ${getButtonStyle(
                  answerKey
                )}`}
              >
                <div className="flex items-center gap-3">
                  <span className="text-amber-300 font-semibold min-w-[100px]">
                    {pair.left}
                  </span>
                  <span className="text-amber-500">→</span>
                  <span className="text-amber-100">{pair.right}</span>
                </div>
              </button>
            );
          })}
      ...
      {showResult && (
        ...
        >
          ...
          {question.explanationBavarian && (
            <p className="text-amber-300/70 text-sm italic mt-1">
              {question.explanationBavarian}
            </p>
          )}
        </div>
